              ...
              if (depth <= max_depth_local &&  depth >= min_depth_local)
                {
                  ...
                  double  temperature_calculated = bottom_temperature_local;

                  temperature_calculated = temperature_calculated + (age > 0 ? (top_temperature_local - bottom_temperature_local)*std::erfc(depth/(2*std::sqrt(thermal_diffusivity*age))) : 0.);
