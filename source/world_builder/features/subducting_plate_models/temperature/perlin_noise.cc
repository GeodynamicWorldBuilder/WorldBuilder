...
#include "world_builder/features/subducting_plate_models/temperature/perlin_noise.h"
    ...
    namespace SubductingPlateModels
    {
      namespace Temperature
      {
        PerlinNoise::PerlinNoise(WorldBuilder::World *world_)
          : min_distance(0.0), max_distance(std::numeric_limits<double>::max()),
            min_temperature(NaN::DSNAN), max_temperature(NaN::DSNAN),
        ...
        PerlinNoise::declare_entries(Parameters &prm, const std::string &)
        {
          ...
          prm.declare_entry("", Types::Object({"min temperature", "max temperature"}),
                            "Perlin noise temperature model. Generates temperature values in a range.");

          // Declare entries of this plugin
          prm.declare_entry("min distance slab top", Types::Double(0),
                            "The distance in meters from which the temperature of this feature is present.");
          prm.declare_entry("max distance slab top", Types::Double(std::numeric_limits<double>::max()),
                            "The distance in meters to which the temperature of this feature is present.");

          prm.declare_entry("min temperature", Types::Double(273.15),
                            "Minimum temperature in degree Kelvin generated by Perlin noise.");
          prm.declare_entry("max temperature", Types::Double(1573.15),
                            "Maximum temperature in degree Kelvin generated by Perlin noise.");
        ...
        PerlinNoise::parse_entries(Parameters &prm)
        {
          ...
          WBAssert(max_distance >= min_distance, "max distance needs to be larger or equal to min distance.");

          min_temperature = prm.get<double>("min temperature");
          max_temperature = prm.get<double>("max temperature");
          WBAssert(max_temperature >= min_temperature, "max temperature needs to be larger or equal to min temperature.");
        ...
        double
        PerlinNoise::get_temperature(const Point<3> &position,
                                     const double,
                                     const double,
                                     double temperature,
                                     const double,
        ...
        {
          if (distance_from_plane.distance_from_plane <= max_distance &&
              distance_from_plane.distance_from_plane >= min_distance)
            {
              const double noise_val = octave_noise(position[0], position[1], position[2], octaves);
              const double new_temperature = min_temperature + (noise_val * 0.5 + 0.5) * (max_temperature - min_temperature);
              return apply_operation(operation, temperature, new_temperature);
            }
          return temperature;
        }
        ...
        WB_REGISTER_FEATURE_SUBDUCTING_PLATE_TEMPERATURE_MODEL(PerlinNoise, perlin_noise)
      }
    }
  }
}
