...
namespace WorldBuilder
{
  namespace Utilities
  {
                                      ...
                                      std::vector<double> obliquity_vector)
    {
      ...
      if (std::isfinite(obliquity_vector[0]) && std::isfinite(obliquity_vector[1]))
        {
          ...
          if (std::isnan(closest_point_on_line_2d[0]))
            {
              // The closest point on the bezier curve has not been found. Extract the terminal points of the bezier curve
              ...
              // Because the bezier curve geometry can be very complex, still track the second closest point. It's unlikely
              // that the second closest point will end up closer to the check point than the first closest point during
              // the iterations, but this may help with edge cases.
              ...
              for (unsigned int i = 0; i < 100; ++i)
                {
                  ...
                  const Point<2> vector_checkpoint_to_curve = closest_terminal_point - check_point_surface_2d_temp_ob;
                  ...
                  if (new_distance_search < closest_distance_to_trench)
                    {
                      // We are getting closer to the curve, update the closest point and distance.
                      ...
                      if (!std::isnan(bezier_curve.closest_point_on_curve_segment(iterable_check_point_surface_2d).point[0]))
                        {
                          // We have found a point on the bezier curve. Set this as the checkpoints closest point on the curve.
                  ...
                  // We are getting farther from the curve, stop the search.
                  else
                    break;
                }
            }

          // Now check if the bezier_curve has found a point on the curve that is closest to the checkpoint (without considering the obliquity vector).
