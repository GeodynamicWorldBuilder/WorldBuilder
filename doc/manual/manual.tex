...
When youare using a model which contains a sticky air layer, you want to make sure that depth 0 is at the surface. This means that the top of the domain is no longer the the surface, so you need to tell the visualizer at what depth you want to set the surface. An example of this case is shown in Listing \ref{lst:code_example_grid_file_sticky_air}.
\begin{bashcode}[label={lst:code_example_grid_file}]{An example of a grid file for a 2d Cartesian grid when you want to use a sticky air free surface of 100km. This file was made to be used in combination with the 2D subduction rift sticky air cookbook.}
# ouput variables
grid_type = cartesian
dim = 2
surface depth = 100e3
